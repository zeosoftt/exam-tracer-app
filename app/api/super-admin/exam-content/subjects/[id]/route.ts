...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (err?.code === 'P2025') return NextResponse.json({ success: false, error: 'Ders bulunamadı.' }, { status: HTTP_STATUS.NOT_FOUND });
    if (err?.code === 'P2002') return NextResponse.json({ success: false, error: 'Bu ders kodu zaten kullanılıyor.' }, { status: HTTP_STATUS.BAD_REQUEST });
    return NextResponse.json({ success: false, error: 'Ders güncellenemedi.' }, { status: HTTP_STATUS.INTERNAL_SERVER_ERROR });
  }
}
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const err = e as { code?: string };
    if (err?.code === 'P2025') return NextResponse.json({ success: false, error: 'Ders bulunamadı.' }, { status: HTTP_STATUS.NOT_FOUND });
    return NextResponse.json({ success: false, error: 'Ders silinemedi.' }, { status: HTTP_STATUS.INTERNAL_SERVER_ERROR });
  }
}

...
export default function OnboardingPage() {
  ...
  const fetchExams = useCallback(async () => {
    try {
      ...
      setExamError(null);
      const result = await fetchAvailableExamsWithStatus();
      if (result.ok) {
        startTransition(() => setAvailableExams(result.exams || []));
      } else {
        setExamError(
          result.errorMessage ||
          ...
          <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center dark:border-red-900/50 dark:bg-red-950/40">
            ...
            <button
              onClick={() => {
                setIsLoadingExams(true);
                setExamError(null);
                void fetchAvailableExamsWithStatus()
                  .then((r) => {
                    if (r.ok) setAvailableExams(r.exams || []);
                    else
                      setExamError(
                        r.errorMessage ||
                          'Sınavlar yüklenemedi. Lütfen sayfayı yenileyin.',

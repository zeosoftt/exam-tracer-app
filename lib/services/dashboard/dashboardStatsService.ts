/**
 * Dashboard istatistik verisi — HTTP/cache route disinda (SRP).
 */
...
async function fetchReviewProgressList(prisma: PrismaClient, userId: string, examId: string): Promise<ReviewProgressRow[]> {
  const topicScope = {
...
export async function buildDashboardStatsData(
  ...
  const { userId, userRole, institutionId, isCoreScope } = input;
    ...
    if (activeExamAssignment?.exam?.id) {
      ...
      const reviewProgressList = isCoreScope ? [] : await fetchReviewProgressList(prisma, userId, examId);
